...
def q_learning():
    ...
    num_episodes = 10000 # number of episodes we'll play
    max_inner_episodes = 3999 # if we have not converged after these many episodes, ditch the qspace and start again
    max_steps_per_episode = 200 # environment auto stops an episode when we hit these many steps
    num_elements_per_dimension = 2 # number of discrete pieces to divide each dimension of the observation space into
    k = 2 # exploration temperature. higher value means we give more weight to explore, else more weight to exploit
    done_reward = -10000 # bad reward for getting done late
    gamma = 0.8  # discount factor. higher value means we still value old data
    min_alpha = 0.99 # minimum learning rate as we keep exploring. alpha will never go lower than this

    # store running average of total reward for last X episodes in a circular buffer
    num_last_few_episodes = 100
    solved_reward_threshold = -110.0
    last_few_rewards = collections.deque(maxlen=num_last_few_episodes)
    min_episodes = 0
    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(max_steps_per_episode):
            ...
            # updating q values
            if done:
                print("\nEpisode {} inner_Episode {} finished after {} timesteps. done is {} learning rate (alpha) = {}"
                    .format(i_episode + 1, num_inner_episodes, t+1, done, alpha))
                print("reward at done: ", reward, total_reward)
                print(q_space)
                if total_reward >= solved_reward_threshold:
                    input()
                ...
                reward = (total_reward - solved_reward_threshold) * 5
        ...
        running_average = sum(last_few_rewards)/num_last_few_episodes
        print("running average of reward per episode: ", running_average)
        if running_average >= solved_reward_threshold and min_episodes >= num_last_few_episodes:
            np.save('./q_space.npy', q_space)
            break
